''' Contains all the necessary functions to run the notebook developers_insights '''
import pandas as pd
...
def create_percentage(df_cur, column_name, column_rename, melt):
    '''
    It creates a new dataframe df_new that contains the percentage of entries that
    correspond to each value of the categorical variable column_name appears. There
    is not calculated one percantage for each entry but two ones, one for women and one for  men.
    INPUT
    df_cur - pandas dataframe
    column_name - name of column (categorical variable) fow which the percentage of
                  its one of each entries will be calculated
    column_rename - how to rename the index of the new dataframe
    melt - True : Adds a categorical column Gender to the dataframe df_new that takes
                  values 'Man' or 'Woman'
           False : creates two columns one for 'Man' and another one for 'Woman'
           OUTPUT
    df_new - new dataframe
    '''
    study_df_women = (df_cur[df_cur['Gender'] == 'Woman'][column_name].value_counts()/
                      len(df_cur[df_cur['Gender'] == 'Woman'][column_name])*100).reset_index()
    study_df_men = (df_cur[df_cur['Gender'] == 'Man'][column_name].value_counts()/
                    len(df_cur[df_cur['Gender'] == 'Man'][column_name])*100).reset_index()
    ...
    if melt:
        df_new = pd.melt(study_df, id_vars=[column_rename], value_vars=['Men', 'Women'],
                         var_name='Gender')
    else:
        df_new = study_df
...
def create_frequency(df_cur, column_name, list_field):
    '''
    Outputs a contingency table for the categorical variable column_name which takes values
    given in list_name and for the Gender variable which takes the values Woman or Man
    INPUT
    df_cur - pandas dataframe
    column_name - the one out of the two categorical variables from which the continengency
                  matrix will be build
...
def plot(y_input, df_cur):
    '''
    Creates a barplot which plots for each entry of the categorical variable
    'y_input' the corresponding numerical value the column 'value' of the dataframe df_cur.
    INPUT
    ...
    axs.legend(loc='center right', frameon=False, prop={'size':20})
...
def chi_squared_test(table):
    ...
    INPUT
    table - contingency table, i.e. table that contains the observed frequencies of the
            categorical variables from which we have built the contingency matrix
    OUTPUT
    ...
    if pvalue <= alpha:
        print('''The p value is: {}, i.e. smaller than the level of signficance p=0.05,
                 thus the null hypothesis is rejected. '''.format(pvalue))
    else:
        print('''The p value is: {}, i.e. larger than the level of signficance p=0.05,
                 thus the null hypothesis is accepted.'''.format(pvalue))
    return pvalue
